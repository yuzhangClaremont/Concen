...
//    }
    ...
    // ctrl+drag line to build a action connection(IBAction) as a funciton. The type is UIButton
    @IBAction func touchCard(_ sender: UIButton) {
        print((cardsButtons.count+1) / 2)
        flipCount += 1
        // if cardNumber is not nil(in set state)
    ...
    func updateViewFromModel(){
        for index in cardsButtons.indices{
            ...
            if card.isFaceUp{
                ...
                button.setTitle("", for: UIControlState.normal)
                // if matched, be clear, otherwise turn back to orange
//                colorLiteral
